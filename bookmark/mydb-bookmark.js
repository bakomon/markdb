...
function mydb_bm_fnc() {
      ...
      el('.db_bmdb_source', 'all').forEach(function(item) {
        item.addEventListener('click', function() {
          if (el('.db_id').value == '') {
            ...
            mydb_open_new_tab(this.dataset.href + el('.db_bmdb').dataset.val);
          }
        });
      });
  ...
  function db_searchResult(arr) {
    ...
    if (arr.length != 0) {
      for (var i = 0; i < arr.length; i++) {
        ...
        s_txt += ' flex_wrap">';
        s_txt += '<div class="_db db_100" onclick="mydb_open_new_tab(\''+ arr[i].url +'\', \'bs_list\')" title="'+ arr[i].url +'">'+ arr[i].title;
        ...
        if ('read' in arr[i] && arr[i].read != '') s_txt += '<button class="_db db_selected" onclick="mydb_open_new_tab(\''+ arr[i].read +'\', \'bs_list\')" title="'+ arr[i].read +'">Read</button>';
  ...
  function db_showHtml(data, note) {
    ...
    s_txt += '<div class="_db db_100"'+ ((wh.indexOf(data.host) == -1 || data.url.indexOf(wp) == -1 || note.indexOf('similar') != -1) && el('title').innerHTML.search(number_t_rgx) == -1 ? ' onclick="mydb_open_new_tab(\''+ data.url +'\', \'bm_list\')"' : '') +' title="'+ data.url +'">';
    if (note != 'similar_one') {
      s_txt += data.title;
    ...
    if (note != 'similar_one') {
      ...
      if ('read' in data && data.read != '') s_txt += '<button class="_db db_selected'+ (chk ? '' : ' db_hidden') +'" onclick="mydb_open_new_tab(\''+ data.read +'\', \'bm_list\')" title="'+ data.read +'">Read</button>';
    ...
    s_txt += '<span class="bm_site db_text'+ (wh.indexOf(data.host) != -1 ? ' db_hidden' : ((not_support && note != 'similar_one') || !mydb_settings.read_project ? ' db_100 t_center' : '')) +'" onclick="mydb_open_new_tab(\''+ data.url +'\', \'bm_list\')">'+ data.host +'</span>';
    s_txt += '</div>';
    s_txt += '</li>';
