...
//function comic_bm() {
  ...
  function db_showHtml(data, note) {
    ...
    s_txt += ' flex_wrap">';
    console.log(data.url.indexOf(wp));
    console.log(wl.href.indexOf(data.url));
    s_txt += '<div class="_db db_100"'+ ((!note && wh.indexOf(data.host) != -1 && el('title').innerHTML.search(number_t_rgx) == -1) || data.url.indexOf(wp) != -1 ? '' : ' onclick="window.open(\''+ data.url +'\')"') +' title="'+ data.url +'">';
