...
(function() {
  ...
  function bc_editData(note, data) {
    ...
    if (el('.bc_cmdb').value == 'none') {
      el('.bc_cm_search_md').dataset.href = '//mangadex.org/search?title='+ data.id.replace(/\-/g, ' ') +'#listing';
      el('.bc_cm_search_mu').dataset.href = '//mangaupdates.com/series.html?search='+ data.id.replace(/\-/g, ' ');
      el('.bc_cm_search_md').classList.remove('bc_hidden');
  ...
  function startBookmark() {
    ...
    b_txt += '<div class="flex bc_100"><input class="bc_url _bc bc_100" type="text" placeholder="URL"><button class="bc_url_open _bc flex f_middle bc_selected bc_hidden"><span>&#128279;</span> Open</button></div>';
    b_txt += '<input class="bc_read _bc bc_100" type="text" placeholder="Link to read (if web to read is different)">';
    b_txt += '<div class="flex bc_100"><input class="bc_image _bc bc_100" type="text" placeholder="Cover image"><button class="bc_img_open _bc flex f_middle bc_selected bc_hidden"><span>&#128444;</span>️ Open</button></div>';
    ...
    b_txt += '<input class="bc_similar _bc bc_100" type="text" placeholder="Similar comic ID">';
    b_txt += '</div>';// .bc_form
    ...
    el('.bc_cm_open').onclick = function() {
      var cm_id = el('.bc_cmdb').value;
    ...
    el('.bc_form_btn .bc_gen').onclick = function() {
      ...
      if (wh.indexOf('mangadex') != -1 && wp.indexOf('/title/') != -1) el('.bc_cmdb').value = wp.match(/\/title\/([^\/]+)/)[1];
      ...
      el('.bc_cm_search_md').dataset.href = '//mangadex.org/search?title='+ comic_id.replace(/[-_\.]/g, ' ') +'#listing';
      el('.bc_cm_search_mu').dataset.href = '//mangaupdates.com/series.html?search='+ comic_id.replace(/[-_\.]/g, ' ');
      if (el('.bc_cmdb').value == '' || el('.bc_cmdb').value == 'none') {
        el('.bc_cm_search_md').classList.remove('bc_hidden');
        el('.bc_cm_search_mu').classList.remove('bc_hidden');
      }
    ...
    el('.bc_form_btn .bc_set').onclick = function() {
      el('.bc_cmdb').value = el('.bc_cmdb').value.toLowerCase();
      ...
      bc_checkData(el('.bc_id').value).then(function(res) {
        if (!res) {
          bc_setData(el('.bc_id').value, el('.bc_cmdb').value, el('.bc_title').value, el('.bc_alt').value, el('.bc_ch').value, el('.bc_note').value, el('.bc_type').value, el('.bc_host').value, el('.bc_url').value, el('.bc_read').value, el('.bc_image').value, el('.bc_last').value, el('.bc_similar').value);
        } else {
    ...
    el('.bc_form_btn .bc_update').onclick = function() {
      ...
      el('.mn_notif span').innerHTML = 'Loading..';
      el('.mn_notif').classList.remove('bc_hidden');
      bc_updateData(el('.bc_id').value, el('.bc_cmdb').value, el('.bc_title').value, el('.bc_alt').value, el('.bc_ch').value, el('.bc_note').value, el('.bc_type').value, el('.bc_host').value, el('.bc_url').value, el('.bc_read').value, el('.bc_image').value, el('.bc_last').value, el('.bc_similar').value);
    };
  }
