...
(function() {
  ...
  function bc_showHtml(data, note) {
    ...
    s_txt += ' flex_wrap">';
    s_txt += '<div class="_bc bc_100'+ (data.similar != '' && main_data[data.similar] && !note ? ' cm_main' : '') +'"'+ (!note && wh.indexOf(data.host) != -1 && el('title').innerHTML.search(chapter_t_rgx) == -1 ? '' : ' onclick="window.open(\''+ data.url +'\')"') +'>'+ data.title;
  ...
  function bc_checkComic(arr, chk) {
    ...
    var title_id = el('title').innerHTML.replace(/&#{0,1}[a-z0-9]+;/ig, '').replace(/\([^\)]+\)/g, '').replace(/\s+/g, ' ').replace(/^baca\s/i, '').replace(/\s(bahasa\s)?indonesia/i, '').replace(/(man(ga|hwa|hua)|[kc]omi[kc])\s/i, '').match(/^([^\-|\||–]+)(?:\s[\-|\||–])?/)[1].replace(/\s$/, '');
