...
(function() {
  ...
  function bc_showHtml(data, note) {
    ...
    s_txt += ' flex_wrap">';
    s_txt += '<div class="_bc bc_100'+ (data.similar != '' && main_data[data.id] && !note ? ' cm_main' : '') +'"'+ (!note && wh.indexOf(data.host) != -1 && el('title').innerHTML.search(chapter_t_rgx) == -1 ? '' : ' onclick="window.open(\''+ data.url +'\')"') +'>'+ data.title;
