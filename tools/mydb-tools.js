const sourceCheck = function(data, prop, note) {
  data = data.replace(/\./g, '-');
...
function mydb_tools() {
  ...
  function genSource() {
    if (source_chk) return;
    firebase.database().ref('bookmark/source').once('value', function(snapshot) {
      /*var data = snapshot.val();*/
      /*var source_txt = '{"anime":'+ genJson(genObject(data.anime)) +',"comic":'+ genJson(genObject(data.comic)) +',"novel":'+ genJson(genObject(data.novel)) +'}';*/
      var source_txt = JSON.stringify(snapshot.val());
      localStorage.setItem('mydb_source_data', source_txt);
      localStorage.setItem('mydb_source_check', 'true');
      source_chk = true;
      callScript();
    });
  }
  ...
  function callScript() {
    mydb_source = JSON.parse(localStorage.getItem('mydb_source_data'));
    ...
    if (mydb_type) {
      ...
      genCSS();
      if (mydb_type == 'comic') comic_cr();//ls_saveLocal('https://bakomon.github.io/page/reader/comic-reader.js', 'mydb_tools_reader', 'js', tools_interval);
      ...
      if (!is_cf && !mydb_read) {
        ...
        comic_bm(); //ls_saveLocal('https://bakomon.github.io/page/bookmark/comic-bookmark.js', 'mydb_tools_bookmark', 'js', tools_interval);
      }
    }
  }
  ...
  /* check project on localStorage */
  if (localStorage.getItem('mydb_source_data') && localStorage.getItem('mydb_source_check') && localStorage.getItem('mydb_source_check') == 'true') {
    source_chk = true;
    callScript();
  } else {
    source_chk = false;
...
var mydb_read = false;
var source_chk = false;
var login_email = '';
var login_pass = '';
var tools_interval = 'manual|7/3/2021, 2:05:26 PM';
